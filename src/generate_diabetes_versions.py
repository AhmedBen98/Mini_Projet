...
class DiabetesDataGenerator:
    ...
    def add_derived_features(self, df):
        ...
        logger.info(f"New features: bmi_category, bp_category, cholesterol_ratio, metabolic_risk, age_group, diabetes_risk")
    ...
    def create_version_1_original(self, output_path):
        ...
        logger.info(f"Version 1 saved: {len(df_enriched)} samples, {len(df_enriched.columns)} features -> {output_path}")
    ...
    def create_version_2_augmented(self, output_path, n_synthetic=100):
        ...
        # Mélanger
        df_augmented = df_augmented.sample(frac=1, random_state=self.random_state).reset_index(drop=True)
        ...
        logger.info(f"Version 2 saved: {len(df_enriched)} samples, {len(df_enriched.columns)} features -> {output_path}")
    ...
    def create_version_3_varied(self, output_path, n_synthetic=200):
        ...
        logger.info(f"Version 3 saved: {len(df_enriched)} samples, {len(df_enriched.columns)} features -> {output_path}")
        logger.info(f"  - Original: {len(df_original)} samples")
